  ...
  class Converter {
    ...
    valueToBinary(value, typeName = null, logStats = null) {
      ...
      const valueType = hasInitialType ?
        ...
        findValueTypeFromValue(value);
      if (!valueType) {
        throw new Error(`Could not find value type from ${hasInitialType ? 'name' : 'value'} "${hasInitialType ? typeName : value}" accepts: ${Object.keys(ValueTypes.nameIndex)}. Leave typeName empty for auto-detect.`);
      }
